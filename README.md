...
`Copy` IDs with O(1) pointer comparison and automatic deduplication. **Those performance properties come from Bevy, not from this
...
- **Deterministic hashing**: `Hash` over the string content rather than the
  interned pointer, so `HashMap`/`HashSet` iteration order is reproducible
  across runs (see [Hashing](#hashing)).
...
- `Hash` - Hashes the string content (see [Hashing](#hashing)); do **not** derive `Hash` yourself
...
Bevy's `Interned<str>` hashes the address of the leaked string allocation.
That address changes from process to process, so a `HashMap`/`HashSet` keyed by
such a value iterates in a different order on every run, even with a
fixed-seed hasher, which breaks deterministic simulation and replay. The
generated `Hash` impl therefore hashes `as_str()` instead: the hash of an ID is
exactly the hash of its `&str` content. Equality stays an O(1) pointer
comparison; interning guarantees equal content if and only if the pointers are
equal, so the `Hash`/`Eq` contract holds. The trade-off is that hashing is
O(n) in the string length instead of O(1).

No `PartialOrd`/`Ord` impl is generated or required.
...
- **Recommended**: `interned_id!(pub MyId);` writes the newtype, the
  `Interned<str>` field, and all five required derives for you. Pass extra
...
- **Manual**: with `#[derive(InternedId)]` directly you must also add
  `Clone`, `Copy`, `PartialEq`, `Eq`, `Debug` yourself (but **not** `Hash`,
  which the derive implements), and wrap `bevy::ecs::intern::Interned<str>`
  explicitly.
...
- **Hashing**: Hashes the string content, O(n) like `String`, so that hashes
  and `HashMap` iteration order are deterministic across runs
...
**Breaking:** `#[derive(InternedId)]` now implements `Hash` itself, hashing the
string content instead of the interned pointer (see [Hashing](#hashing)).

- **Remove `Hash` from any derive list next to `InternedId`**:
  `#[derive(InternedId, Clone, Copy, PartialEq, Eq, Hash, Debug)]` becomes
  `#[derive(InternedId, Clone, Copy, PartialEq, Eq, Debug)]`. Leaving it in
  fails to compile with a conflicting `Hash` implementation error (E0119).
- Types declared with `interned_id!` need no changes.
- Hash values change: they no longer depend on memory addresses, so
  `HashMap`/`HashSet` iteration order over IDs is now the same on every run
  (for a hasher with a fixed seed). Any persisted or cached hash values from
  0.5 are not comparable with 0.6 values.
